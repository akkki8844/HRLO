...
class SessionController:
    ...
    def _save(self):
        ...
        emg_path = os.path.join(
            self.output_dir, f"{self.participant_id}_emg_{ts}.csv"
        )
        btn_path = os.path.join(
            self.output_dir, f"{self.participant_id}_buttons_{ts}.csv"
        )

        with open(emg_path, "w", newline="") as f:
            writer = csv.writer(f)
            header = ["timestamp"] + [f"ch{i}" for i in range(self.emg.n_channels)]
            writer.writerow(header)
            for s in self.emg.get_samples():
